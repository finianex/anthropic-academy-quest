...
/* ── 練習 ───────────────────────────────────────────────────
   一堂課的題目從筆記結構推導。**一個概念一場只考一次**：配對題先挑走
   一批概念，剩下的才出選擇題，再加上每條誤區一題的辨識題。
   實測 154 篇筆記的概念數是 3–7、誤區 3–5 條，所以每堂 6–8 題。 */
export const QUIZ = {
  optionCount: 4,      // 選擇題的選項數（含正解）
  matchPairs: 4,       // 配對題最多幾對
  /* 誤區辨識出幾題。**只能是 2，一個方向各一題。**
     誤區題只有兩種問法（「哪個是誤區」「哪個是建議做法」），出到第三題
     必然和前兩題其中之一題幹相同；而選項池又只有那 3 條誤區與幾個流程
     步驟，兩題並排看起來就是同一題換了順序。實測出 3 題時 100% 的場次
     都有重複題幹，改成 2 題後歸零。 */
  pitfallMax: 2,
